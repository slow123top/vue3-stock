...
const str = (leftBracket, rightBracket, arr, includeLogo) => {
  let tempA = '';
  let end = 0;
  arr.forEach(function (ele, index) {
    ...
    if (ele.className === '') {
      ...
      tempA += ele.className + '-';
      //A0008 A0009
      if (Object.prototype.hasOwnProperty.call(ele, 'select1') && ele.select1.length !== 0) {
...
const splitABC = (para, selectedIndexs, symbol) => {
  ...
  if (index.indexName === 'A0008' || index.indexName === 'A0009') {
    ...
    if (Object.prototype.hasOwnProperty.call(myModelIndexs[index.indexName], 'radios') && myModelIndexs[index.indexName].radios.length !== 0) {
      selectedIndexs[selectedIndexs.length - 1].radios.push({
        value: index.indexValue[index.indexValue.length - 1],
        threeFlag: myModelIndexs[index.indexName].radios[0].threeFlag,
        label1: myModelIndexs[index.indexName].radios[0].label1,
...
export const indicatorToDes = (arr, leftSymbol, rightSymbol) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (everySelectedIndex.select1.length !== 0) {
      ...
      //专门为C0001写的模型信息查看
      if (Object.prototype.hasOwnProperty.call(everySelectedIndex, 'selects') && everySelectedIndex.selects.length !== 0) {
